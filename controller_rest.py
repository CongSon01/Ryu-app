...
class NetworkStatRest(ControllerBase):
    ...
    def getTopo(self, req, **_kwargs):
        hosts, switches, links = self.app.topology_data.get_topology_data()

        value_devices = [{"id": "of:" + str(switch['dpid']), "type":"SWITCH"} for switch in switches]

        value_hosts = [{"mac":str(host['mac']) ,
        ...
        value_links = [{"src": {"port": int(link["src"]["name"].split("eth")[1]), "id":"of:" + str(link['src']['dpid']) },
